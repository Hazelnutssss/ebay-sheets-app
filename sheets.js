...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      const cellVal = (rows[i][1] || '').toString().trim(); // Spalte B
    ...
    await gFetch(`${sheetsBase}/values:batchUpdate`, token, 'POST', {
      valueInputOption: 'USER_ENTERED',
      data: updates.map(u => ({
        range: encodeSheetRange(STOCK_SHEET, u.range.split('!')[1]),
        values: [[u.value]]
      }))
    });
    ...
    await gFetch(`${sheetsBase}/values:batchUpdate`, token, 'POST', {
      valueInputOption: 'USER_ENTERED',
      data: [{
        range: encodeSheetRange(STOCK_SHEET, `H${sheetRowNum}`),
    ...
    await gFetch(`${sheetsBase}/values:batchUpdate`, token, 'POST', {
      valueInputOption: 'USER_ENTERED',
      data: [{
        range: encodeSheetRange(SOLD_SHEET, `A${firstEmptyInSold}`),
        values: [rowData]
      }]
    });
